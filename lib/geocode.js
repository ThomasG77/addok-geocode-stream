...
function prepareItem(item, {columns, citycode, postcode}) {
  const preparedItem = {
    [getKey('line_number')]: item[getKey('line_number')],
...
function expandItemWithResult(item, resultItems, resultColumns = []) {
  const id = item[getKey('line_number')]
  const resultItem = resultItems[id]

  return {
    ...(omit(item, getKey('line_number'))),
...
async function batchGeocode(serviceURL, items, options = {}) {
  ...
  const indexedResultItems = keyBy(resultItems, getKey('line_number'))
